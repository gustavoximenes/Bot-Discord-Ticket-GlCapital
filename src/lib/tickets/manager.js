...
module.exports = class TicketManager {
	...
	async finallyClose(ticketId, {
		...
		// also send the close summary (+ transcript button) to any configured transcript channels
		try {
			const transcriptChannels = JSON.parse(ticket.guild.transcriptChannels || '[]');
			for (const channelId of transcriptChannels) {

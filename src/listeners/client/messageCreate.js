		...
		if (message.channel.type === ChannelType.DM) {
			...
			if (ticket) {
				...
				if (!message.author.bot) {
					...
					// when the ticket opener replies, restart the whole follow-up sequence:
					// reset both the timer and the number of reminders already sent
					if (ticket.reminder && message.author.id === ticket.createdById) {
						try {
							const reminder = JSON.parse(ticket.reminder);
